...
export default class Header extends Component {
    ...
    _handleKeyDown = (e) => {
        if (e.key === 'Enter' && this.state.isLoaded) { 
            ...
            this.setState({ activeSuggestion: this.state.activeSuggestion - 1, scrollDownwards: false });
        }
        else if (e.keyCode === 40) { // Down arrow
            e.preventDefault();
            if (this.state.activeSuggestion < this.state.suggestionList.length-1) {
                this.setState({ activeSuggestion: this.state.activeSuggestion + 1, scrollDownwards: true });
            }
        }
    }
    ...
    componentDidUpdate() {
        ...
        if (el) {
            ...
            if (!isVisible) { el.scrollIntoView(!this.state.scrollDownwards); }
        }
    }
                    ...
                    {this.state.suggestionList.map((suggestion, index) => {
                        ...
                        <li className={className} key={suggestion.id} onMouseDown={() => this.onClick(suggestion)}>
                            ...
                            <div className="suggestion-info">
                                ...
                                <p>{media.title.romaji} {mediaYear}</p>
                            </div>
                        </li>
                        );
                    })}
